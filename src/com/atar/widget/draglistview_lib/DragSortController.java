...
public class DragSortController extends SimpleFloatViewManager implements View.OnTouchListener, GestureDetector.OnGestureListener {
		...
		if (mCanDrag && !mDragging && (mHitPos != MISS || mFlingHitPos != MISS)) {
			if (mHitPos != MISS) {
				if (mDragInitMode == ON_DRAG && Math.abs(y2 - y1) > mTouchSlop && mSortEnabled) {
					...
					mIsRemoving = true;
					if (mOnRemoveingDoingListener != null) {
						mOnRemoveingDoingListener.OnRemoveingDoing(mIsRemoving, mFlingHitPos - mDslv.getHeaderViewsCount(), deltaX);
						return true;
					}
					if (listener != null) {
						listener.OnRemoveing(mIsRemoving);
					}
	 ...
	 * 正在移动删除直接滑动删除 
	 * @author :Atar
	 * @createTime:2014-9-10上午11:38:02
	 * @version:1.0.0
	 * @modifyTime:
	 * @modifyAuthor:
	 * @description:
	 */
	public interface OnRemoveingListener {
		void OnRemoveing(boolean IsRemoving);
	}

	private OnRemoveingListener listener;

	public void setOnRemoveingListener(OnRemoveingListener listener) {
	 ...
	 * 正在移动时时操作其它事情，item不动
	 * @author :Atar
	 * @createTime:2014-9-10下午2:46:14
	 * @version:1.0.0
	 * @modifyTime:
	 * @modifyAuthor:
	 * @description:
	 */
	public interface OnRemoveingDoingListener {
		void OnRemoveingDoing(boolean IsRemoving, int position, int deltaX);
	}
