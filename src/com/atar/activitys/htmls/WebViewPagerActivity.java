 ...
 *****************************************************************************************************************************************************************************
 */
public class WebViewPagerActivity extends AtarCommonWebViewPagerActivity implements OnPageChangeListener {
	private String VIEWPAGER_OPTION_JSON_KEY = "VIEWPAGER_OPTION_JSON_KEY";

	@Override
	protected void onCreate(Bundle bundle) {
		setExtendsAtarCommonWebViewPagerActivity(false);
	...
	public void onClick(View v) {
		...
		case R.id.txt_common_top_left:
			try {
				((AtarDynamicFragment) getFragmentList().get(getCurrentItem())).onBackPressed();
			} catch (Exception e) {
				e.printStackTrace();
			}
			break;
		...
		case R.id.txt_common_top_right:// 顶部右边文字
			try {
				((AtarDynamicFragment) getFragmentList().get(getCurrentItem())).loadWebViewUrl("javascript:onClickTopRight('" + getCurrentItem() + "')");
			} catch (Exception e) {
			}
			break;
		default:
