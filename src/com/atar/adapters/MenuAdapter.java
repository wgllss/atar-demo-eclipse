...
public class MenuAdapter extends CommonAdapter<TabMenuItemBean> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (info != null) {
			...
			if (getCurrentPostiotn() == position) {
				// LoadUtil.setTextColor(getContext(), mViewHolder.menu_title, R.array.select_tab_txt_color, getSkinType());
			} else {
				if (info.getMenuNameColor() != null && info.getMenuNameColor().length() > 0 && info.getMenuNameColor().contains(",")) {
					...
					// LoadUtil.setTextColor(getContext(), mViewHolder.menu_title, R.array.tab_txt_color, getSkinType());
				}
			}
		}
		convertView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (info != null && 4034 == info.getID()) {
					// ((HomeActivity) getContext()).sendEmptyMessage(EnumMsgWhat.REFRESH_HANDLER5);
				}
